...
| `--check` | выкл | проверить исходники на соответствие ЕСПД; код выхода 1 при ошибках |
...
(PDF/DOCX/TXT/HTML/MD/JSON), тип документа (19.301…19.505 или `all`),
флаги нейросетей и ZIP, лог генерации.

## Проверка кода (--check)

```bash
gostdoc --check --project ./src
```

Инспекция на соответствие ЕСПД без генерации документов: недокументированные
классы/методы, незавершённый код (`TODO`/`FIXME`/`HACK`), чрезмерная
вложенность блоков, длинные строки, избыточные ветвления. Код выхода 1 —
если найдены ошибки (удобно для CI).
...
pip install dist/gostdoc-0.6.0-py3-none-any.whl
```
