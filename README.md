...
| `--format` / `-f` | `pdf,docx,txt` | форматы: pdf, docx, txt, html, md, json |
...
| `--doctype` | `19.402` | тип документа: `19.301`, `19.401`, `19.402`, `19.403`, `19.404`, `19.504`, `19.505` или `all` |
| `--zip` | выкл | упаковать все сгенерированные документы в ZIP |
| `--init` | — | создать шаблон `.gostdoc.json` в текущем каталоге |
...
- Markdown-рендер (для GitHub) с таблицами и схемами в `figures/`;
...
Шаблон конфигурации создаётся командой `gostdoc --init`.
Приоритет параметров: явные аргументы CLI > конфигурация > значения по умолчанию.
